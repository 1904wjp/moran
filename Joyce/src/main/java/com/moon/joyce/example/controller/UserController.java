...
/*********************************************************************************************************************************************/
    ...
    public String websocketPage(ModelMap map) {
...
//        }
        ...
        return dataResult(isRedisConnection, "发送失败,redis暂未开通", "发送成功");
    }
    ...
    public Result loginUser(@RequestParam("username") String username, @RequestParam("password") String password, HttpServletRequest request) {
        try {
            getRedisValueOperation().get(Constant.SESSION_USER);
        } catch (Exception e) {
            logger.error("redis服务器启动异常:{}",e.getMessage());
            isRedisConnection=false;
        }
        logger.info("rc------------->{}", isRedisConnection);
        String logIp = HttpUtils.getIpAddress(request);
